...
Although dimension data doesn't change frequently, it does change slowly - and how we choose to store
that history is critical for accurate analytics.

Why is SCD important?
    Because in analytics, sometimes we :
    * Only care about the latest dimension values
    * But often we want to track historical changes to understand how data looked at a point in time
...
------------------------------------------------------------------------------------------------
    ...
    * No new rows - new columns are added to hold previous value
...
Delta Lake MERGE is commonly used to implement SCD
